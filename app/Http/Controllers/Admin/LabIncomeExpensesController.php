...
use App\Models\LabIncomeExpense;
use App\Models\Treasury;
use App\Models\TreasurySteps;
use App\Traits\GeneralTrait;
use Illuminate\Http\Request;

class LabIncomeExpensesController extends Controller
{
    use GeneralTrait;

    public function show()
    {
        $LabIncomeExpense = LabIncomeExpense::where('softDelete', 0)->get();
        return response()->json($LabIncomeExpense);
    }
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            if($request->kind == 1){
                $LabIncomeExpense_id = LabIncomeExpense::create([
            ...
            if ($request->payingKind == 1) {
                TreasurySteps::create([
                    'kind' => $kind,
                    'kind_id' => $LabIncomeExpense_id->id,
    ...
    {
        $LabIncomeExpense = LabIncomeExpense::find($id);
        ...
        try {
            ...
            $total = LabIncomeExpense::where('id', $id)->value('total');
            if ($request->payingKind == 1) {
                $balance = Treasury::where('id', $request->payingKind_id)->value('balance');
                if ($balance < $total) {
            ...
            if($request->kind == 1){
                LabIncomeExpense::where('id',$id)->update([
    ...
    {
        $total = LabIncomeExpense::where('id', $id)->value('total');
        $payingKind = LabIncomeExpense::where('id', $id)->value('payingKind');
        $payingKind_id = LabIncomeExpense::where('id', $id)->value('payingKind_id');
        $lab_id = LabIncomeExpense::where('id', $id)->value('lab_id');
        $user_id = LabIncomeExpense::where('id', $id)->value('user_id');
        $note = LabIncomeExpense::where('id', $id)->value('note');
        if ($payingKind == 1) {
